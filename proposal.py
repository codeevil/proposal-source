...
### 4. 策略排序规则
生成的 **{PROPOSAL_COUNT} 个执行策略，必须严格按照「高召回率 + 高QPS（低时延）」的综合性价比，从高到低按序组织排列**，规则如下：
1. **编号与排名对应**：`strategy_id` 与综合排名一一对应，`strategy_id = 1` 为综合表现最优的策略，编号越大，综合性价比越低。
2. **综合评估依据**：结合当前标量条件的选择性数值，同时量化考量策略的预期召回率水平与预期查询延迟（QPS表现），优先将「收益代价比最高、召回与性能平衡度最优」的策略排在前列。
3. **排序分层要求**：
   - 前 1/3 位次：优先放置兼顾召回与性能的平衡型最优策略，是默认场景下的首选方案
   - 中间 1/3 位次：放置不同技术方向、单一目标偏向的差异化策略，覆盖不同业务侧重
   - 后 1/3 位次：放置极端偏好（极致QPS/极致召回）、执行代价较高或适用场景受限的策略
