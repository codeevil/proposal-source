...
### 策略排序规则
生成的 **{PROPOSAL_COUNT} 个执行策略，必须严格遵循「召回率优先、同召回比性能」的规则，从优到劣按序组织排列**，具体规则如下：
1. **编号与排名对应**：`strategy_id` 与综合排名一一对应，`strategy_id = 1` 为整体优先级最高的策略；编号越大，综合优先级越低。
2. **排序双维度准则**：
   - 第一判断维度：**预期召回率**，按预期召回率从高到低排序，召回率更高的策略始终排在更前位次，不受性能指标影响
   - 第二判断维度：**预期QPS（时延）**，仅当两个策略的预期召回率处于同一水平、大体持平时，再按预期QPS从高到低（时延从低到高）排序
3. **排序分层要求**：
   - 前 1/3 位次：优先放置高召回梯队中时延表现最优的策略，是对召回质量有硬性要求场景下的首选方案
   - 中间 1/3 位次：放置中等召回水平、性能表现各有侧重的差异化策略，覆盖不同业务权衡场景
   - 后 1/3 位次：放置低召回、极致QPS导向的策略，或适用场景受限、执行代价较高的特殊策略
